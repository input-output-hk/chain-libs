        ...
        match content {
            ...
            Fragment::VoteCast(tx) => {
                ...
                let account_id = match tx
                    ...
                    .unwrap()
                {
                    (InputEnum::AccountInput(account_id, _), Witness::Account(_)) => account_id
                        .to_single_account()
                        .ok_or(Error::AccountIdentifierInvalid)?,
                    (_, _) => {
...
fn input_single_account_verify<'a>(
    ...
    let new_ledger = ledger.remove_value(account, value)?;
    ledger = new_ledger;
...
fn input_multi_account_verify<'a>(
    ...
    // .remove_value() check if there's enough value and if not, returns a Err.
    let (new_ledger, _declaration) = ledger.remove_value(account, value)?;
