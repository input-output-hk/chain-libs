...
pub fn vote_plan_in_block0() {
    ...
    let committee = Wallet::from_value(Value(0)).private_key();
    let committee_id = committee.to_public().into();

    let vote_plan = VoteTestGen::vote_plan();
    let vote_plan_certificate = create_initial_vote_plan(&vote_plan, &[alice.clone()], committee);

    let ledger_build_result =
        LedgerBuilder::from_config(ConfigBuilder::new(0).with_committee(&[committee_id]))
