...
impl Zkp {
    ...
    pub(crate) fn generate<R: RngCore + CryptoRng>(
        ...
        let response = {
            ...
            let p1 = cipher_randoms.iter().zip(cy_powers.iter()).fold(
                Scalar::zero(),
                |acc, (r, cy_pows)| {
            ...
            let p2 = rs
                .iter()
                .zip(cx_pows.iter())
                .fold(Scalar::zero(), |acc, (r, cx_power)| {
                    acc + r * cx_power
                });
            p1 + p2
        };
    ...
    fn verify_statements(
        ...
        let products_z_mul_powers_cy_sum =
            powers_cy.iter().zip(products_z_iter)
                .fold(Scalar::zero(), |sum, (cy_i, z_i)|{
                    sum + &z_i * cy_i
                });
        ...
        let powers_cy_mul_cx_log_n = powers_cy.iter().map(|cy_i| cy_i * &cx_log_n).collect::<Vec<_>>();

        let batch_challenge2 = Scalar::random(&mut thread_rng());

        GroupElement::vartime_multiscalar_multiplication(
            powers_cy_mul_cx_log_n.iter().map(|p| p * &batch_challenge2).collect::<Vec<_>>().into_iter()
                .chain(powers_cx.iter().map(|p| p * &batch_challenge2).collect::<Vec<_>>())
                .chain(iter::once(products_z_mul_powers_cy_sum - self.r.clone() * &batch_challenge2))
                .chain(powers_cy_mul_cx_log_n)
                .chain(powers_cx)
                .chain(iter::once(self.r.negate())),
            ciphertexts.iter().map(|ct| ct.e1.clone())
...
mod tests {
    ...
    fn false_proof() {
        ...
        let ciphertexts_swapped = ciphertexts.into_iter()
            .map(|ct|Ciphertext{e1: ct.e2, e2: ct.e1}).collect::<Vec<_>>();
